...
/*输出扫描结果*/
	...
	// prepare output to style with CSS  修改函数
	function highlightline($tokens=array(), $comment='', $line_nr, $title=false, $udftitle=false, $tainted_vars=array())
	{
									...
									} else
									{
										foreach($key as $token)
										{
											if(is_array($token))
											{
												...
												{
													$cssname = str_replace('$', '', $token[1]);
													//$text.= $cssname.$cssname;
													//$text.= $cssname.$cssname;
												}
												$text .= $token[1];
												//$text .= str_replace('_', '-', strtolower(token_name($token[0]))).htmlentities($token[1], ENT_QUOTES, 'utf-8');
											}	
											else
												$text .= $token;
										}
		...
		if(!empty($comment))
			$output .= " //".$comment;//htmlentities($comment, ENT_QUOTES, 'utf-8');   注释部分内容
		return $output;
	}
	...
	// traced parameter output top-down    修改函数
	...
	// 打印搜索结果
	// print the scanresult  修改函数
	function printoutput($output, $treestyle=2)
	{
		if(!empty($output))
		{
			$nr=0;
