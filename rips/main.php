...
**/
	...
	//$_POST['loc'] = "/usr/local/nginx/html/sql2.php";
	
	$_POST['loc'] = $argv[1];	
	if(empty($argv['vector'])){
		$_POST['vector'] = "all";
	}else{
		$_POST['vector'] = $argv['vector'];
	}	 
    $_POST['subdirs'] = 1;
    $_POST['verbosity'] = 1;   
    $_POST['treestyle'] = 2;
	
	//var_dump($_POST['loc']);exit;
	...
	{		
		...
		if(is_dir($location))
		{
            ...
            if(count($files) > WARNFILES && !isset($_POST['ignore_warning'])){
				...
				echo "警告：有",count($files),"文件需要扫描，可能需要一段时间，你确定要扫描这个目录吗?","\n";
				fwrite(STDOUT,"是否继续扫描:Y/N ?");  //标准输出，等待用户输入 Y/N
				$flag = trim(fgets(STDIN));           //标准输入，立即获取用户输入
				if($flag!="Y")exit;
			}	
			*/					
		}	
		...
		if(empty($_POST['search']))
		{
			...
			for($fit=0; $fit<$file_amount; $fit++)
			{
				...
				//var_dump($file_scanning);   var_dump($scan_functions);    var_dump($info_functions);   var_dump($source_functions);exit;    
